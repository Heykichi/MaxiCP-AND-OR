...
public class MapColoring_And_PS {
    public static void main(String[] args) {
        String path = (args.length >= 1) ? args[0] : "graph_coloring/regions/world";
        int limite = (args.length >= 2) ? Integer.parseInt(args[1]) : 1000000;
        ...
        DFSearchMini_And_PS search;
        switch (searchType) {
            case 2:
                search = cp.dfSearchMini_And_PS(graph, naiveTreeBuilding(graph,fix_to_split,to_fix), firstFail());
                break;
            default:
                search = cp.dfSearchMini_And_PS(graph, fiducciaMattheyses(graph,to_fix, true), firstFail());
